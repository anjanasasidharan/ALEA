...
public class PBS_PRO implements SchedulingPolicy {
    ...
    public void addNewJob(GridletInfo gi) {
        ...
        if (Scheduler.data_set.equals("metacentrum.mwf")) {
    ...
    /** PBS-pro like algorithm. It is setted up according to the situation used in Czech Grid infrastructure "MetaCentrum".<p>
     * For sharcnet, the setup would be different. Therefore, we do not suggest that you use it as it is but rather take it
     * as a inspiration how multi-queue system with priorities may be implemented.
     */
    @Override
    public int selectJob() {
        ...
        for (int q = 0; q < Scheduler.all_queues.size(); q++) {
            ...
            Collections.sort(curr_queue, new WallclockComparator());
            ...
            for (int i = 0; i < curr_queue.size(); i++) {
                ...
                if (r_cand != null) {
                    gi = (GridletInfo) curr_queue.remove(i);
                    ...
                    if (gi.getNumPE() > r_cand.resource.getNumPE()) {
                        System.out.println("PBS error : " + gi.getID());
